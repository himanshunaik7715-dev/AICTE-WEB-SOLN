...
export default async function handler(req: any, res: any) {
  ...
  const [{ data: actorProfile }, { data: actorWhitelist }] = await Promise.all([
    db.from('users').select('id,role').eq('email', actorEmail).maybeSingle(),
  ...
  const key = isEmail ? userIdOrEmail.toLowerCase() : userIdOrEmail;
  const [{ data: profile }, { data: admin }] = await Promise.all([
    isEmail ? db.from('users').select('*').eq('email', key).maybeSingle() : db.from('users').select('*').eq('id', key).maybeSingle(),
    isEmail ? db.from('admins').select('*').eq('email', key).maybeSingle() : db.from('admins').select('*').eq('id', key).maybeSingle(),
  ]);
  ...
  if (profile) {
    const { error } = await db.from('users').update({ tgmApprovalStatus: 'rejected' }).eq('id', profile.id);
    if (error) return res.status(500).json({ success: false, error: error.message });
  }
  if (admin) {
    const { error } = await db.from('admins').update({ isWhitelisted: false, approvalStatus: 'rejected' }).eq('id', admin.id);
    if (error) return res.status(500).json({ success: false, error: error.message });
  }
  return res.status(200).json({ success: true, preservedAsStudent: false, email: profile?.email || admin?.email });
}
