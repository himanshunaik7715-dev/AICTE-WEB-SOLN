 ...
 ELSIF TG_OP='UPDATE' THEN
  ...
  IF auth.role() IS DISTINCT FROM 'service_role' AND public.current_app_role() IS DISTINCT FROM 'superadmin' THEN
   ...
   IF OLD.role='student' THEN
    protected_new:=protected_new-ARRAY['name','erpNo','rollNo','department','course','division'];
    protected_old:=protected_old-ARRAY['name','erpNo','rollNo','department','course','division'];
...
CREATE TABLE portal_private.profile_edit_audit(id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,student_id text NOT NULL,old_profile jsonb NOT NULL,new_profile jsonb NOT NULL,edited_at timestamptz NOT NULL DEFAULT now());
REVOKE ALL ON portal_private.profile_edit_audit FROM PUBLIC,anon,authenticated;
CREATE FUNCTION public.edit_student_profile(patch jsonb) RETURNS public.users
LANGUAGE plpgsql SECURITY DEFINER SET search_path=public,pg_temp AS $$
...
BEGIN
 ...
 SELECT * INTO s FROM public.academic_scopes WHERE id=(patch->>'scope_id')::bigint AND enabled AND academic_batch=u."academicBatch" FOR SHARE;
 IF NOT FOUND THEN RAISE EXCEPTION 'Select a valid academic option for your batch'; END IF;
 UPDATE public.users SET name=trim(patch->>'name'),"erpNo"=patch->>'erpNo',"rollNo"=trim(patch->>'rollNo'),"phoneNumber"=patch->>'phoneNumber',department=s.department,course=s.course,division=s.division WHERE id=u.id RETURNING * INTO saved;
...
INSERT INTO public.portal_migrations VALUES('005_direct_student_profile',now());
COMMIT;
